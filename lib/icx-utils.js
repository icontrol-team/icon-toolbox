...
export async function get_fee(json_rpc){
    // let json_rpc = editor_obj.get();
    let step_limit = "";
    if (json_rpc && json_rpc.params && json_rpc.params.from) {
        ...
        let promise = new Promise((resolve, reject) => {
            call_api_payload(new iconTemplateMethod().get('getStepCosts'), "/api/v3",false).then(function (data) {
                ...
                call_api_payload(new iconTemplateMethod().get('getStepPrice'), "/api/v3", false).then(function (data) {
                    ...
                    call_api_payload(json_rpc, "/api/v3d", false).then(function (data) {
                        if (data.result.status_code === 400) {
                            ...
                            let step_limit_wei = estimate_step * step_price;
...
export function tx_sign(){
    if (arrayContains(getIdValue("methods"), AVAIL_SIGN_METHODS)) {
        ...
        // get_fee().then( function(step_limit){
        ...
        // json_rpc.params.stepLimit = step_limit || "0x4a817c800";
        ...
        // let step_limit = get_fee();

        get_fee(json_rpc)
            .then( function(step_limit){
                json_rpc.params.stepLimit = step_limit || DEFAULT_STEP_LIMIT;
