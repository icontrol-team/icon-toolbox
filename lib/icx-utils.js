...
import {AVAIL_SIGN_METHODS, INS_SETTINGS} from "./config";
...
export function get_ins_score_address(){
    // let selected_network = getNetworkInfo(state).network_name;
    let selected_network = getNetworkInfo().network_name;
    let ins_setting_info = INS_SETTINGS[selected_network];

    if (ins_setting_info) {
        // logging_msg("network_name= " + selected_network+ ", ins_score_address= " + ins_setting_info.score_address);
        return INS_SETTINGS[selected_network].score_address;
    }else{
        console.log("network_name = " + selected_network+ ", INS score not found");
    }
    return undefined;
}
...
export function set_INS_config(payload_to_change={}){
    // console.log("changed INS settings:", payload_to_change);
        ...
        if (selected_method && selected_group === "INS") {
            ...
            switch (selected_method) {
                case "registerINS":
                    ...
                    payload_to_change.params.data.params._address = getKeystore().address;
                    break;
                // case "setPrice":
                //     payload_to_change.params.value = "0x0";
                //     break;
